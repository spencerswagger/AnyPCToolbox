  ...
  <div class="space-y-4">
    ...
    <div class="httpd-panel">
      <div class="httpd-panel-title">
        <span class="httpd-eyebrow text-muted-foreground">variables</span>
        <span class="text-xs text-muted-foreground">（{{ Object.keys(resolved).length }}）</span>
        <span class="ml-auto flex items-center gap-2">
          <label v-if="props.api.variables.some((v) => v.name === 'page')" class="flex items-center gap-1.5 text-xs text-muted-foreground">
            page
            <input type="number" min="1" class="w-16 rounded border border-border bg-background px-2 py-0.5 text-xs focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
              :value="page" :disabled="running" @change="goToPage(Number(($event.target as HTMLInputElement).value) || 1)" />
          </label>
          <button class="httpd-btn httpd-btn-accent rounded px-4 py-1.5 text-xs" :disabled="running" @click="send">
    ...
    <div v-if="err" class="rounded border-l-2 border-l-destructive border-border bg-destructive/10 px-3 py-2 font-mono text-xs text-destructive">
      <span class="font-bold">! err · </span>{{ err }}
    </div>
    ...
    <div v-if="result" class="httpd-panel">
      <div class="flex items-center gap-3 border-b border-border px-3 py-2 text-xs">
        ...
        <div class="ml-auto flex gap-1">
          <button class="rounded px-2 py-0.5 text-xs font-medium" :class="view === 'raw' ? 'bg-accent' : 'hover:bg-accent'" @click="view='raw'">raw</button>
          <button class="rounded px-2 py-0.5 text-xs font-medium" :class="view === 'table' ? 'bg-accent' : 'hover:bg-accent'" @click="view='table'">table</button>
        </div>
      </div>
      ...
      <div v-else-if="parsed && !parsed.ok" class="p-3 font-mono text-xs text-muted-foreground">
        <span class="text-destructive">{{ parsed.error }}</span><span v-if="parsed.topKeys?.length">；top-level: {{ parsed.topKeys.join(', ') }}</span>
      </div>
